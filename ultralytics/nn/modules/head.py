...
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "Detect_CSTDHead"
...
class Detect_CSTDHead(nn.Module):
    ...
    def __init__(self, nc=80, hidc=256, ch=()):
        ...
        self.cls_decoup = TaskDecomposition(hidc, n, 16)
        self.reg_decoup = TaskDecomposition(hidc, n, 16)

        self.DyDCNV2 = DyDCNv2(hidc, hidc)
        self.spatial_conv_offset = nn.Conv2d(n * hidc, 3 * 3 * 3, 3, padding=1)
        self.offset_dim = 2 * 3 * 3

        self.SFMconv = nn.Conv2d(n, 1, kernel_size=5, padding=2)  # 7,3
    ...
    def forward(self, x):
        """Concatenates and returns predicted bounding boxes and class probabilities."""
        feats = []  # 存储每个 Conv_GN 层的输出
        n = 2
        for i in range(self.nl):
            feat = x[i]
            for conv in self.share_conv:
                feat = conv(feat)
                feats.append(feat)

            # 使用 torch.cat 拼接n个特征图
            feat = torch.cat(feats[-n:], dim=1)  # 拼接最后特征图
            # print('feat.shape:', feat.shape)

            # task decoupling
            avg_feat = F.adaptive_avg_pool2d(feat, (1, 1))
            cls_feat = self.cls_decoup(feat, avg_feat)
            reg_feat = self.reg_decoup(feat, avg_feat)
            ...
            # cls alignment
            avg_feat_3d = F.adaptive_avg_pool3d(feat, (n, feat.size(2), feat.size(3)))
            cls_prob = self.SFMconv(avg_feat_3d).sigmoid()


            x[i] = torch.cat((self.scale[i](self.cv2(reg_feat)), self.cv3(cls_feat * cls_prob)), 1)
            # x[i] = torch.cat((self.scale[i](self.cv2(reg_feat)), self.cv3(cls_feat)), 1)
        if self.training:  # Training path
            return x
